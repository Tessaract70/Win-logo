...
on both the host and server. For the server side, a registry entry must be made at HKEY_CURRENT_USER\
...
long as it's reflected in the string. For the host side you must install Ansicon in order for your
...
VER%==^%ANSICON_VER^% "c:\Program Files\Ansicon\x64\ansicon" -p)" without the quotes. Lastly the
directory from which you executed the ansicon -i command must be added to the system path in the
Environment Variables. Happy SSHing!
